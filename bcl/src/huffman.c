...
/* The maximum number of nodes in the Huffman tree is 2^(16+1)-1 = 131071 */
#define MAX_TREE_NODES 131071
...
/*************************************************************************
* _Huffman_Read8Bits() - Read 16 bits from a bitstream.
*************************************************************************/
...
static void _Huffman_Hist( unsigned short *in, huff_sym_t *sym,
  unsigned int size )
{
  int k;
  ...
  huff_bitstream_t *stream, unsigned int code, unsigned int bits )
{
  ...
  if( node->Symbol >= 0 )
  {
    ...
    /* Find symbol index */
    for( sym_idx = 0; sym_idx < 65536; ++ sym_idx )
    {
...
int Huffman_Compress( unsigned short *in, unsigned short *out,
  unsigned int insize )
{
  huff_sym_t       sym[65536], tmp;
...
void Huffman_Uncompress( unsigned short *in, unsigned short *out,
  unsigned int insize, unsigned int outsize )
{
  ...
  for( k = 0; k < outsize; ++ k )
  {
    ...
    *buf ++ = (unsigned short) node->Symbol;
  }
}
